...
import { IBlog, IUser } from "@/types/interfaces";
...
const Dashboard: React.FC<DashboardProps> = ({ user, loading }) => {
  ...
  const changeBlogStatus = async (blogId: string, status: string) => {
    updateBlog(blogId, { status })
      .then(() => mutate())
      .catch(() => toast.error("Something went wrong..."));
  };

  const createOption = (blogStatus: string) => {
    return blogStatus === "draft"
      ? { view: "Publish Story", value: "published" }
      : { view: "Make a Draft", value: "draft" };
  };

  const createOptions = (blog: IBlog) => {
  ...
  const renderBlogs = (blogs: IBlog[], status: string) => (
